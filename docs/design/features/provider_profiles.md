...
named pools. The unqualified provider name selects `default`, preserving every
existing route, model ID, key index, and configuration shape.
...
The registry resolves a selector into the existing provider adapter plus an
immutable credential-profile view. Adapter instances remain safe to share
...
Gateway still receives the base provider name because profiles are a proxy-side
...
Gateway-ready credentials retain the same index alignment as ordinary keys
inside each profile.

Model aggregation enumerates the default provider plus every configured named
profile. Default IDs retain `<provider>/<model>` while named IDs use
...
Auxiliary provider settings remain shared. Examples include Azure resource and
