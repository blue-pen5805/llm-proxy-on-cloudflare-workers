...
UI. The UI groups deployment settings by purpose, masks credential input and
existing secret values, shows effective defaults in field hints, validates the
result against the configuration schema, and preserves comments in an existing
JSONC file. Provider credentials are selected by provider name rather than
