...
Run `npm run secrets` to create or edit the default file through the terminal
UI. Its first prompt selects English or Japanese for the current session. The
UI groups deployment settings by purpose, masks credential input and existing
secret values, shows effective defaults in field hints, validates the result
against the configuration schema, and preserves comments in an existing JSONC
file. Provider credentials are selected by provider name rather than
configuration binding name. Provider-specific settings, such as the Azure
OpenAI resource and API version or the AWS Bedrock region, appear in the same
provider submenu as its credential. API-key inputs are labeled `API_KEY`;
token- and service-account-based providers use labels that identify those
specific inputs. The top-level sections are ordered as Proxy authentication,
Providers, Custom endpoints, Virtual models, Cloudflare AI Gateway, and
Behavior. `DEV` is intentionally not editable in the UI because it is a local
authentication bypass rather than a deployment setting; its template or
existing value is preserved unchanged. Each field offers an explicit **Keep
current value (no change)** action in addition to changing or clearing it. Esc
returns one level and Ctrl+C exits without saving. Run the command again
whenever you need to change the file. For a named environment, use:

```bash
npm run secrets -- --env production
```

When `CLOUDFLARE_ACCOUNT_ID` is unset, the UI runs
`wrangler whoami --json` and uses its account as the default. If Wrangler
returns several accounts, the UI asks which one to use. The Account ID remains
editable, so the detected value can be replaced manually. If Wrangler is not
authenticated or account discovery fails, configuration continues without a
default.
